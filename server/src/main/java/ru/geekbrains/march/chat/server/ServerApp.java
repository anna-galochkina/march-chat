...
//            иначе если выводить сообщение о количестве тут, то клиент получает сообщение не связанное с командой /stat и воспринимает его как свое и увеличивает счетчик сообщений
//            пробовала тут создать статическую переменную, увеличивать ее, когда сервер отправляет эхо и уже на клиенте брать ее значение
//            (чтобы можно было по разным командам подчитывать сообщения всего и от клиента /total_stat и /stat), но в переменной всегда по какой то причине 0.
//
            while (true) {
                String msg = in.readUTF().trim();
                if (msg.length() > 0) {
